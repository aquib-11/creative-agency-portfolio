...
import { getIndustryById, updateIndustry } from "@/services/industryservices";
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (i.coverImage?.url) setImagePreview(i.coverImage.url);
      } catch {
        setNotFound(true);
      } finally {
        setFetching(false);
  ...
  if (notFound) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-background gap-4">
        <p className="text-foreground font-medium">Industry not found</p>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                placeholder="e.g. Gyms, trainers, and wellness centers"
                className="w-full px-4 py-2.5 rounded-md border border-border bg-background text-foreground placeholder:text-muted-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary transition-colors"
              />
            </div>
